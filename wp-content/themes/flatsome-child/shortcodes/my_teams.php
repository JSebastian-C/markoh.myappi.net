...
<div class="fake_body">
	<?php if(count($equipos)):?>
		<?php foreach($equipos as $v):?>
			...
			<div class="noticia">
				<div class="image_container" style="background-image:url(<?=$v->logo_url?>);"></div>
				<?php if($v->editable):?>
				...
				<div class="jugadores">
					...
					<?php foreach($jugadores as $j):?>
						<?php 
							$imagen = get_user_meta($j->id_player,'profile_picture')[0]; 
						...
						<div class="jugador"  style="position:relative;margin-top:10px;padding-left:40px;">
							<div class="image_container" style="background-image:url(<?=$magen?>);top:0;left:0px;height:30px;width:30px;"></div>
							<b><?=$nombre . " " . $apellido ?>.</b> 
	...
	jQuery(function($){	
		$('a.eliminar_equipo').on('click',function(e){
			...
			if(confirm("¿Desea eliminar el equipo '" + $(this).data('nombre') + "'?")){
				$.post("/wp-admin/admin-ajax.php?action=custom_ajax&caction=delete_team&id="+$(this).data('id')),null,function(r){
					alert(r.message);
